...
public class MapChecker {
	...
	public String validateNewFence(Map map, Point firstEnd, Point secondEnd) {
		...
		String error = "";
		Fence fence = new Fence(firstEnd, secondEnd);
		//check if the length of the wall = 1 
		if (fence.getLength() != 1) {
			error = "fence length should be = 1";
			return error;
		}
		//check if the wall is only horizontal or vertical
		if (! ((fence.getFirstEnd().getX() == fence.getSecondEnd().getX()) || (fence.getFirstEnd().getY() == fence.getSecondEnd().getY()))  ) {
			error = "a fence shoud be horizontal or vertical";
			return error;
		}
		...
		if (map.getFenceList().contains(new Fence(firstEnd, secondEnd))) {
	...
	private Graph buildGraph() {
		...
		for (int i=0; i<map.getLength()-1; i++) {
			for (int j=0; j<map.getLength();j++) {
				graph.add(new Edge(new Point(i,j),new Point(i+1,j)));
			}
		}
		//add vertical edges
		for (int i=0; i<map.getLength(); i++) {
			for (int j=0; j<map.getLength()-1;j++) {
				graph.add(new Edge(new Point(i,j),new Point(i,j+1)));
			}
		}
	...
	private boolean searchPaths(String colour,Point pawnLocation, Graph graph) {
		...
		if (colour.equals("blue")) {
			point = new Point(0,map.getLength()-1);
		}
		else {
			point = new Point(0,0);
			}
